 ...
 */
class Order31ActivitiesStarter implements ActivitiesStarter {

    @Override
    public void startActivityA(final Context context) {
        final Intent intent = new Intent(context, ActivityA.class);
        intent.putExtra(BaseActivity.ORDER_KEY, Order.ORDER31.name());
    ...
    public void startFinalActivity(final Context context) {
        Toast.makeText(context, "Nothing to do", Toast.LENGTH_LONG).show();
    }
}
